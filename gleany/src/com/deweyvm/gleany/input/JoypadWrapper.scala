...
object JoypadWrapper:
  ...
  def getAxisList(value: Any/*PovDirection*/): List[Axes.Axis] = ??? //port TODO
    /*value match
      ...
      case PovDirection.northEast =>
        getAxisList(PovDirection.north) ++ getAxisList(PovDirection.east)
      case PovDirection.northWest =>
        getAxisList(PovDirection.north) ++ getAxisList(PovDirection.west)
      case PovDirection.southEast =>
        getAxisList(PovDirection.south) ++ getAxisList(PovDirection.east)
      case PovDirection.southWest =>
        getAxisList(PovDirection.south) ++ getAxisList(PovDirection.west)
*/
class JoypadWrapper(controller: Controller):
  private val buttonState =
    new mutable.HashMap[Int, Boolean]().withDefault(_ => false)
  ...
  private val listener = new ControllerAdapter:
    ...
    override def axisMoved(
        controller: Controller,
        axisCode: Int,
        value: Float
    ): Boolean =
      if hasPov then return false
    ...
    def povMoved(
      ...
      JoypadWrapper.getAxisList(value) foreach {
        (axis: JoypadWrapper.Axes.Axis) =>
