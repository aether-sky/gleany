...
object Color:
  ...
  def fromHsb(
    ...
    val hsv = java.awt.Color.getHSBColor(
      hue.toFloat,
      saturation.toFloat,
      brightness.toFloat
    )
  ...
  def fromRgba(r: Double, g: Double, b: Double, a: Double): Color =
    Color(r, g, b, a)
...
case class Color(r: Double, g: Double, b: Double, a: Double = 1):
  private val libgdxColor =
    new LibgdxColor(r.toFloat, g.toFloat, b.toFloat, a.toFloat)

  def toLibgdxColor: LibgdxColor = libgdxColor
  def toByteTuple: (Int, Int, Int) =
    ((r * 256).toInt, (g * 256).toInt, (b * 256).toInt)

  def dim(factor: Double): Color = Color(r / factor, g / factor, b / factor)
  def mult(factor: Double): Color = Color(r * factor, g * factor, b * factor)
